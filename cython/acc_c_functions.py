...
import acc_c_helpers as c_helpers
...
def get_epochs(sigmag, locf = True):
    ...
    epoch_dev, epoch_samples = c_helpers.proc_epochs(np.asarray(times), np.asarray(samples), 
    ...
    # do LOCF imputation
    if locf:
        temp_dict['locf'] = np.asarray( c_helpers.proc_locf(epoch_dev, epoch_samples) )
...
def analyze_activity(directory, plot = True):
    ...
    for i in range(len(output)):
        ...
        # call c extension        
        times = get_times( np.asarray(activity) )
